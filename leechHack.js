export async function main(ns) {
    ...
    const gMessage = 'Server is too poor - We should attempt to grow, but we will hack anyway.';
    ...
    while (true) {
        ...
        if (action == 3) { await ns.hack(target) }
        else if (action == 2) { await ns.hack(target) }
